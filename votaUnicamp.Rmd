...
nao_total=rep(0,24)
for(i in 1:24){
 nao_total[i]=c(sum(mytbl$nao[which(institutos==inst[i])])) 
}

sim_total=rep(0,24)
for(i in 1:24){
 sim_total[i]=c(sum(mytbl$sim[which(institutos==inst[i])])) 
}

abs_total = rep(0,24)
for(i in 1:24){
 abs_total[i]=c(sum(mytbl$abstencao[which(institutos==inst[i])])) 
}
...
total_inst=rep(0,24)
for(i in 1:24){
 total_inst[i]=c(sum(mytbl$total[which(institutos==inst[i])])) 
}

dados=cbind(inst,nao_total, sim_total, abs_total, propn_total, props_total, propabs_total, total_inst)
```
